...
namespace TopoCS
{
    class Texture
    {
        ...
        public bool Initialize(Device device, string filename)
        {
            try
            {
                using (var texture = LoadFromFile(device, new SharpDX.WIC.ImagingFactory(), filename))
                {
                    ...
                    _texture = new ShaderResourceView(device, texture, srvDesc);
                    device.ImmediateContext.GenerateMips(_texture);
                }
            }
            catch
            {
        ...
        static public Texture2D CreateTexture2DFromBitmap(Device device, BitmapSource bitmapSource)
        {
